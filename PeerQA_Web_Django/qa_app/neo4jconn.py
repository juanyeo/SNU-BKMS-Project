...
class Database2():
    ...
    def update(self):
        ...
        node1 = apoc_string + '"qa_app_user") YIELD row MERGE (u:User {id: toInteger(row.id), uname:row.first_name})'
        ...
        query = 'MATCH (a:User)-[:ASK]->(:Question)<-[:Relate]-(:Comment)<-[:Answer]-(b:User) WHERE a.id <> b.id  WITH a.id as id1, b.id as id2, count(b.id)/6 as cnt MATCH (c:User{id:id1}), (d:User{id:id2}) MERGE (c)-[:Friends{strength:cnt}]->(d)'
